...
import { Manager } from 'src/@core/types'
...
export const CreateNewAccountModal = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
  ...
  return (
    <Dialog open={open}>
      <DialogTitle textAlign='center'>Create New Manager</DialogTitle>
      <IconButton
        aria-label='close'
        onClick={onClose}
        sx={{
...
const Example = (props: any) => {
  ...
  const [tableData, setTableData] = useState<Manager[]>([])

  const [createModalOpen, setCreateModalOpen] = useState(false)
  const [deleteModalOpen, setDeleteModalOpen] = useState(false)
  const [rowDel, setRowDel] = useState<number>()

  const handleCreateNewRow = (values: Manager) => {
    console.log(values)
    fetch(`https://cheapr.my.id/manager/`, {
      method: 'POST',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleSaveCell = (cell: MRT_Cell<Manager>, value: any) => {
    ...
    const newData: any = [...tableData]
    newData[cell.row.index][cell.column.id as keyof Manager] = value
    const pk = newData[cell.row.index]['pk']
    const payload: Payload = {}
    payload[key as keyof Manager] = value
    setTableData([...newData])

    fetch(`https://cheapr.my.id/manager/${pk}/`, {
      method: 'PATCH',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
...
export default withAuth(3 * 60)(Manager)
