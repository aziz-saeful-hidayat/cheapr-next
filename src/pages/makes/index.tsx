...
interface CreateModalProps {
  columns: MRT_ColumnDef<ProductMakes>[]
  onClose: () => void
  onSubmit: (values: ProductMakes) => void
  open: boolean
}
...
export const CreateNewAccountModal = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
  ...
  return (
    <Dialog open={open}>
      <DialogTitle textAlign='center'>Create New Product Maker</DialogTitle>
      <IconButton
        aria-label='close'
        onClick={onClose}
        sx={{
...
const Example = (props: any) => {
  ...
  const { data, isError, isFetching, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const fetchURL = new URL('/make/?ordering=id', 'https://cheapr.my.id')
  ...
  const [tableData, setTableData] = useState<ProductMakes[]>([])

  const [createModalOpen, setCreateModalOpen] = useState(false)
  const [deleteModalOpen, setDeleteModalOpen] = useState(false)
  const [rowDel, setRowDel] = useState<number>()

  const handleCreateNewRow = (values: ProductMakes) => {
    console.log(values)
  ...
  const handleDeleteRow = (row: number) => {
    fetch(`https://cheapr.my.id/make/${row}/`, {
      method: 'DELETE',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleSaveCell = (cell: MRT_Cell<ProductMakes>, value: any) => {
    const key = cell.column.id
    const oldData = [...tableData]
    const newData: any = [...tableData]
    newData[cell.row.index][cell.column.id as keyof ProductMakes] = value
    const pk = newData[cell.row.index]['pk']
    const payload: Payload = {}
    payload[key as keyof ProductMakes] = value
    setTableData([...newData])

    fetch(`https://cheapr.my.id/make/${pk}/`, {
      method: 'PATCH',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const columns = useMemo<MRT_ColumnDef<ProductMakes>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'Name'
...
export default withAuth(3 * 60)(ProductMakes)
