...
interface CreateModalProps {
  columns: MRT_ColumnDef<ProductCondition>[]
  onClose: () => void
  onSubmit: (values: ProductCondition) => void
  open: boolean
}
...
export const CreateNewAccountModal = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
  ...
  return (
    <Dialog open={open}>
      <DialogTitle textAlign='center'>Create New Product Condition</DialogTitle>
      <IconButton
        aria-label='close'
        onClick={onClose}
        sx={{
...
const Example = (props: any) => {
  ...
  const [tableData, setTableData] = useState<ProductCondition[]>([])

  const [createModalOpen, setCreateModalOpen] = useState(false)
  const [deleteModalOpen, setDeleteModalOpen] = useState(false)
  const [rowDel, setRowDel] = useState<number>()

  const handleCreateNewRow = (values: ProductCondition) => {
    console.log(values)
    fetch(`https://cheapr.my.id/product_condition/`, {
      method: 'POST',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleDeleteRow = (row: number) => {
    fetch(`https://cheapr.my.id/product_condition/${row}/`, {
      method: 'DELETE',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleSaveCell = (cell: MRT_Cell<ProductCondition>, value: any) => {
    const key = cell.column.id
    const oldData = [...tableData]
    const newData: any = [...tableData]
    newData[cell.row.index][cell.column.id as keyof ProductCondition] = value
    const pk = newData[cell.row.index]['pk']
    const payload: Payload = {}
    payload[key as keyof ProductCondition] = value
    setTableData([...newData])

    fetch(`https://cheapr.my.id/product_condition/${pk}/`, {
      method: 'PATCH',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const columns = useMemo<MRT_ColumnDef<ProductCondition>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'Name'
...
const ProductCondition = (props: any) => (
...
export default withAuth(3 * 60)(ProductCondition)
