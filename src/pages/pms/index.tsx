...
interface CreateModalProps {
  columns: MRT_ColumnDef<PM>[]
  onClose: () => void
  onSubmit: (values: PM) => void
  open: boolean
}
...
export const CreateNewAccountModal = ({ open, columns, onClose, onSubmit }: CreateModalProps) => {
  ...
  return (
    <Dialog open={open}>
      <DialogTitle textAlign='center'>Create New PM</DialogTitle>
      <IconButton
        aria-label='close'
        onClick={onClose}
        sx={{
...
const Example = (props: any) => {
  ...
  const { data, isError, isFetching, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const fetchURL = new URL('/product_manager/?ordering=id', 'https://cheapr.my.id')
  ...
  const [tableData, setTableData] = useState<PM[]>([])

  const [createModalOpen, setCreateModalOpen] = useState(false)
  const [deleteModalOpen, setDeleteModalOpen] = useState(false)
  const [rowDel, setRowDel] = useState<number>()

  const handleCreateNewRow = (values: PM) => {
    console.log(values)
    fetch(`https://cheapr.my.id/product_manager/`, {
      method: 'POST',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleDeleteRow = (row: number) => {
    fetch(`https://cheapr.my.id/product_manager/${row}/`, {
      method: 'DELETE',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleSaveCell = (cell: MRT_Cell<PM>, value: any) => {
    const key = cell.column.id
    const oldData = [...tableData]
    const newData: any = [...tableData]
    newData[cell.row.index][cell.column.id as keyof PM] = value
    const pk = newData[cell.row.index]['pk']
    const payload: Payload = {}
    payload[key as keyof PM] = value
    setTableData([...newData])

    fetch(`https://cheapr.my.id/product_manager/${pk}/`, {
      method: 'PATCH',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const columns = useMemo<MRT_ColumnDef<PM>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'Name'
...
export default withAuth(3 * 60)(PMs)
