...
const Example = (props: any) => {
  ...
  const { data, isError, isFetching, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const fetchURL = new URL('/archive_items/', 'https://cheapr.my.id')
  ...
  const handleCreateNewRow = (values: Item) => {
    console.log(values)
    fetch(`https://cheapr.my.id/archive_items/`, {
      method: 'POST',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
  ...
  const handleSaveCell = (cell: MRT_Cell<Item>, value: any) => {
    ...
    setTableData([...newData])
    console.log(payload)
    fetch(`https://cheapr.my.id/archive_items/${pk}/`, {
      method: 'PATCH',
      headers: new Headers({
        Authorization: `Bearer ${session?.accessToken}`,
        'Content-Type': 'application/json'
