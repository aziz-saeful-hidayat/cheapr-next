...
interface CreateModalProps {
  columns: MRT_ColumnDef<ReturnToSeller>[]
  onClose: () => void
  onSubmit: (values: ReturnToSeller) => void
  open: boolean
  channelData: any[]
}
interface AddItemProps {
  ...
  open: boolean
  rowData: ReturnToSeller | undefined
  roomData: Room[]
}
...
const Example = (props: any) => {
  ...
  const [tableData, setTableData] = useState<ReturnToSeller[]>(() => data?.results ?? [])
  const [channelData, setChannelData] = useState<Channel[]>([])
  const [roomData, setRoomData] = useState<Room[]>([])

  const [createModalOpen, setCreateModalOpen] = useState(false)
  const [addModalOpen, setAddModalOpen] = useState<ReturnToSeller>()
  ...
  const handleCreateNewRow = (values: ReturnToSeller) => {
    console.log(values)
  ...
  const handleSaveRow: MaterialReactTableProps<ReturnToSeller>['onEditingRowSave'] = async ({
    exitEditingMode,
    row,
    values
  }) => {
  ...
  const handleSaveCell = (cell: MRT_Cell<ReturnToSeller>, value: any) => {
    ...
    if (key === 'channel.name') {
      ...
      newData[cell.row.index][cell.column.id as keyof ReturnToSeller] = value
    }
  ...
  const columns = useMemo<MRT_ColumnDef<ReturnToSeller>[]>(
    () => [
      {
