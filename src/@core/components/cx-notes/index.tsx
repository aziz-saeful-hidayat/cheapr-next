...
export const CxNotes = ({ open, onClose, sales, session, reload }: AddItemProps) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://cheapr.my.id/open_issue/${sales}/`, {
      method: 'get',
      headers: new Headers({
      ...
      .then(json => {
        setInput(json.cs_comment ? json.cs_comment : '')
      })
      .finally(() => {
        setLoading(false)
      })
  ...
  const handleSend = () => {
    fetch(`https://cheapr.my.id/open_issue/${sales}/`, {
      ...
      body: JSON.stringify({ cs_comment: input })
    })
      .then(response => response.json())
      .then(json => {})
      .finally(() => {
